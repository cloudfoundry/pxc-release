...
var _ = Describe("Emitter", func() {
	Describe("Handler", func() {
		...
		It("Responds with backend connection metrics", func() {
			...
			Expect(body).To(ContainElement("# HELP backend_session_total Gauge of the current sessions from this proxy to a mysql backend"))
			Expect(body).To(ContainElement("# TYPE backend_session_total gauge"))
			Expect(body).To(ContainElement(`backend_session_total{backend="backend-0"} 19`))
			Expect(body).To(ContainElement(`backend_session_total{backend="backend-1"} 11`))
			Expect(body).To(ContainElement(`backend_session_total{backend="backend-2"} 216`))
		})
	})
})
