...
func (s *starter) StartNodeFromState(state string) (string, <-chan error, error) {
	...
	case Clustered:
		...
		newNodeState = Clustered
	case Halted:
		mysqldChan, err = s.startThenHalt()
		newNodeState = Clustered
	default:
...
func (s *starter) startThenHalt() (chan error, error) {
	s.logger.Info("Starting then halting")
	cmd, err := s.dbHelper.StartThenHaltMysqld()
	...
	s.mysqlCmd = cmd
	s.logger.Info("Issuing a non-blocking Wait for mysqld in exit in Halt mode")
	mysqldChan := s.osHelper.WaitForCommand(cmd)
	// HALT ==> Stop monit from looping this command execution
	out, err := s.osHelper.RunCommand("monit", "stop", "galera-init")
	if err != nil {
		return nil, fmt.Errorf("Failed to monit stop galera-init: %w\n\tcommant output: %s", err, out)
	}
