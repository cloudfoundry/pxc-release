...
func (h httpClusterHealthChecker) HealthyCluster() bool {
	...
	for _, url := range h.clusterUrls {
		h.logger.Info("Checking if node is healthy", "url", url)

		resp, err := h.client.Get(url)
		if err != nil {
			h.logger.Error("checking cluster member health failed", "error", err)
			continue
		}
		if resp != nil && resp.StatusCode == 200 {
			h.logger.Info("node is healthy - cluster is healthy.", "url", url)
			return true
		}
		body, _ := io.ReadAll(resp.Body)
		h.logger.Info("node is NOT healthy",
			"url", url,
			"status", resp.Status,
			"body", string(body),
		)
