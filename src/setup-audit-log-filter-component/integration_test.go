...
import (
	...
	"setup-audit-log-filter-component/internal/testing/docker"
)
...
var _ = Describe("AuditLog Installation Workflow", Label("docker", "integration", "workflow"), func() {
	...
	BeforeEach(func() {
		once.Do(func() {
			var err error
			setupBinPath, err = gexec.BuildWithEnvironment("setup-audit-log-filter-component", []string{"CGO_ENABLED=0"})
			Expect(err).NotTo(HaveOccurred())
		})

		var err error
		mysqlContainer, err = docker.RunContainer(docker.ContainerSpec{
			Image: "percona/percona-xtradb-cluster:8.4.3-3.1-multi",
	...
	It("enables audit logging", func() {
		...
		Expect(structuredLog).To(ConsistOf(gstruct.MatchAllKeys(gstruct.Keys{
