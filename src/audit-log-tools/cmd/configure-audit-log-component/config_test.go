...
import (
	main "auditlogtools/cmd/configure-audit-log-component"
...
var _ = Describe("ParseConfig", func() {
	It("parses config", func() {
		...
		GinkgoT().Setenv("MYSQL_AUDIT_LOG_FILTER", `{"filter":{"log":true}}`)
	...
	When("no default audit log filter is specified", func() {
		It("returns an error", func() {
			var cfg main.Config

			err := main.ParseConfig(&cfg)
			Expect(err).To(MatchError(ContainSubstring(`required environment variable "MYSQL_AUDIT_LOG_FILTER" is not set`)))
		})
	})
})
