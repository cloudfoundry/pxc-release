...
describe 'galera-agent-config template' do
  ...
  supported_plugins.each do |tc|
    context 'valid user_authentication_policy configuration' do
      before { spec["engine_config"] = { "user_authentication_policy" => tc[:plugin] } }
  ...
  unsupported_plugins.each do |tc|
    context 'unsupported user_authentication_policy configuration' do
      ...
      it "raises an error for #{tc[:plugin]}" do
        expect { template.render(spec) }.to raise_error(RuntimeError, "Unsupported value '#{tc[:plugin]}' for 'engine_config.user_authentication_policy' property. Choose from 'mysql_native_password' or 'caching_sha2_password'")
      end
    end
  end
end
